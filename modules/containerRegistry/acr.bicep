...
module pid_cuaId './nested/nested_cuaId.bicep' = if (!empty(cuaId)) {
  name: 'pid-${cuaId}'
  params: {}
}
...
module registry_rbac './nested/nested_rbac.bicep' = [for (roleAssignment, index) in roleAssignments: {
...
module registry_privateEndpoints './nested/nested_privateEndpoints.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
